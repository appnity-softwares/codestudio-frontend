...
export function ChangelogSection() {
    return (
        <section id="changelog" className="py-24 px-6 bg-canvas relative overflow-hidden">
            {/* Background Glow */}
            ...
            <div className="container max-w-7xl mx-auto relative z-10">
                <div className="flex flex-col md:flex-row items-end justify-between gap-6 mb-16">
                    <div className="max-w-xl">
                        <h2 className="text-3xl font-bold mb-4 flex items-center gap-3 text-foreground">
                    ...
                    <Link to="/changelog">
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            className="group flex items-center gap-2 px-6 py-3 rounded-xl bg-card border border-border hover:border-primary/50 transition-all text-sm font-medium text-foreground"
                        >
                            View Full Changelog
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {CHANGES.map((change, i) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className="group p-6 rounded-2xl bg-card border border-border hover:border-primary/20 transition-all flex flex-col h-full shadow-sm"
                        >
                            <div className="flex items-start justify-between mb-6">
                                ...
                                <div className="text-right">
                                    <Badge variant="outline" className={cn(
                                        ...
                                        change.type === 'BREAKING' ? 'text-red-500 border-red-500/20' :
                                            change.type === 'FEATURE' ? 'text-emerald-500 border-emerald-500/20' :
                                                'text-blue-500 border-blue-500/20'
                                    )}>
                                        {change.type}
                                    </Badge>
                                    <div className="text-[10px] text-muted-foreground font-mono italic">{change.date}</div>
                                </div>
                            </div>
                            ...
                            <div className="flex-1">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <h3 className="font-bold text-foreground group-hover:text-primary transition-colors">{change.title}</h3>
                                </div>
