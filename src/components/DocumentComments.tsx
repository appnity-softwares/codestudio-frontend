...
export function DocumentComments({ documentId }: { documentId: string }) {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            ...
            setIsLoading(true);
            try {
                const data = await documentsAPI.getComments(documentId);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const { comment } = await documentsAPI.comment(documentId, optimisticComment.content);
                    ...
                    {isLoading ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50 p-8">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-xs text-white/40 max-w-[200px]">Start the conversation.</p>
                            </div>
                        </div>
                    ) : (
