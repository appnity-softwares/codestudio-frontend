        ...
        <div className="container max-w-7xl mx-auto py-10 px-6 space-y-10 animate-in fade-in duration-700">
                ...
                <div className="flex items-center gap-4">
                    <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                        <DialogTrigger asChild>
                            <Button className="h-12 px-6 rounded-2xl bg-white text-black hover:bg-white/90 font-bold shadow-xl shadow-white/5">
                                <Plus className="w-5 h-5 mr-2" />
                                Create Track
                            </Button>
                        </DialogTrigger>
                        <DialogContent className="bg-[#0c0c0e] border-white/10 text-white rounded-[2rem] max-w-md">
                            <DialogHeader>
                            ...
                            <form onSubmit={handleCreate} className="space-y-6 pt-4">
                                <div className="space-y-2">
                                    ...
                                    <Input name="title" placeholder="e.g. Master Go Concurrency" required className="h-12 bg-white/5 border-white/10 rounded-xl" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-[10px] font-black uppercase tracking-widest text-white/40">Learning Objective</Label>
                                    <Textarea name="description" placeholder="What will learners achieve?" className="bg-white/5 border-white/10 rounded-xl min-h-[100px]" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-[10px] font-black uppercase tracking-widest text-white/40">Difficulty Tier</Label>
                                ...
                                <Button type="submit" disabled={createMutation.isPending} className="w-full h-14 rounded-2xl bg-primary text-white font-black uppercase tracking-wider">
            ...
            <div className="relative group max-w-2xl">
                ...
                <Input
                    ...
                    className="h-14 pl-12 bg-white/[0.03] border-white/5 rounded-2xl focus:ring-primary/20 transition-all font-medium"
                />
            </div>
            ...
            {isLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {[1, 2, 3].map(i => (
                        <div key={i} className="h-80 rounded-3xl bg-white/[0.02] border border-white/5 animate-pulse" />
                    ))}
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {data?.playlists?.map((playlist: any, i: number) => (
                        ...
                        >
                            <Link to={`/roadmaps/${playlist.id}`} className="group block h-full">
                                <div className="relative h-full p-8 rounded-[2rem] bg-gradient-to-br from-white/[0.05] to-transparent border border-white/5 group-hover:border-primary/30 transition-all duration-500 overflow-hidden flex flex-col">
                                    {/* Accent Decoration */}
                                    ...
                                    <div className="space-y-3 mb-8 flex-1 relative z-10">
                                        <h3 className="text-2xl font-black text-white group-hover:text-primary transition-colors font-headline leading-tight">
                                            {playlist.title}
                                        </h3>
                                    ...
                                    <div className="pt-6 border-t border-white/5 flex items-center justify-between relative z-10">
                                        <div className="flex items-center gap-4 text-[11px] font-bold text-white/30 uppercase tracking-tight">
                                        ...
                                        <div className="h-10 w-10 rounded-full border border-white/10 flex items-center justify-center group-hover:bg-white group-hover:border-white transition-all duration-500">
